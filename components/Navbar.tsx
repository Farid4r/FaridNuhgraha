...
import { usePathname } from "next/navigation"; // Hook untuk cek halaman

export default function Navbar() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isDarkMode, setIsDarkMode] = useState(false);
  const pathname = usePathname(); // Deteksi kita lagi di halaman mana
    ...
    <nav className="fixed w-full top-0 z-50 bg-white/80 dark:bg-dark/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 transition-colors duration-300">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center relative">
        ...
        <Link href="/" className="z-10 flex items-center">
          <img src="/assets/logo-light.png" alt="Farid Logo Light" className="h-8 md:h-10 w-auto object-contain transition-transform duration-300 hover:scale-105 block dark:hidden" />
          <img src="/assets/logo-dark.png" alt="Farid Logo Dark" className="h-8 md:h-10 w-auto object-contain transition-transform duration-300 hover:scale-105 hidden dark:block" />
        </Link>

        <div className="flex items-center gap-4 md:gap-8 z-10">
          {/* Desktop Menu - Menggunakan Rute Asli */}
          <div className="hidden md:flex gap-8 font-medium text-sm dark:text-gray-300">
            <Link href="/" className={`${pathname === "/" ? "text-primary" : ""} hover:text-primary transition-colors`}>Home</Link>
            <Link href="/design" className={`${pathname === "/design" ? "text-primary" : ""} hover:text-primary transition-colors`}>Design</Link>
            <Link href="/video" className={`${pathname === "/video" ? "text-primary" : ""} hover:text-primary transition-colors`}>Video</Link>
          </div>
      ...
      <div className={`absolute top-full left-0 w-full bg-white dark:bg-dark border-b border-gray-100 dark:border-gray-800 shadow-lg transform transition-all duration-300 ease-in-out md:hidden flex flex-col px-6 py-6 gap-6 text-center text-lg font-medium ${isMenuOpen ? "translate-y-0 opacity-100 pointer-events-auto" : "-translate-y-full opacity-0 pointer-events-none"}`}>
        <Link href="/" onClick={toggleMenu} className="text-dark dark:text-white hover:text-primary">Home</Link>
        <Link href="/design" onClick={toggleMenu} className="text-dark dark:text-white hover:text-primary">Design</Link>
        <Link href="/video" onClick={toggleMenu} className="text-dark dark:text-white hover:text-primary">Video</Link>
      </div>
    </nav>
  );
}