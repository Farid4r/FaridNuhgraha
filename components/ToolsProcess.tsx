export default function ToolsProcess() {
  return (
    // Menghapus bg-light dan card background agar menyatu dengan halaman
    <section className="py-24 md:py-32 border-t border-gray-200 dark:border-gray-800 transition-colors duration-300">
      <div className="container mx-auto px-6">
        
        {/* Mengubah layout menjadi flexbox antara Judul dan Konten */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-12 about-reveal">
          
          {/* Sisi Kiri: Judul dengan font Serif */}
          <div className="w-full md:w-1/3">
            <h2 className="text-4xl md:text-5xl font-serif italic font-medium tracking-tight mb-4 dark:text-white">
              Software.
            </h2>
            <p className="text-gray-500 dark:text-gray-400 font-light text-base md:text-lg">
              Alat tempur digital yang saya gunakan untuk mewujudkan ide.
            </p>
          </div>

          {/* Sisi Kanan: Daftar Tools tanpa dibungkus kotak (card) */}
          <div className="w-full md:w-2/3 flex flex-wrap gap-4 md:justify-end">
            
            {/* Tag 1: Photoshop */}
            {/* Mengubah rounded-full menjadi rounded-md, dan uppercase font */}
            <span className="group flex items-center gap-3 px-6 py-3 md:px-8 md:py-4 border border-gray-200 dark:border-gray-800 rounded-md text-xs uppercase tracking-widest font-light hover:border-black dark:hover:border-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all duration-500 cursor-default">
              <svg className="w-5 h-5 text-gray-400 group-hover:text-white dark:group-hover:text-black transition-colors duration-500" fill="currentColor" viewBox="0 0 24 24">
            ...
            {/* Tag 3: CorelDraw */}
            <span className="group flex items-center gap-3 px-6 py-3 md:px-8 md:py-4 border border-gray-200 dark:border-gray-800 rounded-md text-xs uppercase tracking-widest font-light hover:border-black dark:hover:border-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all duration-500 cursor-default">
              <svg className="w-5 h-5 text-gray-400 group-hover:text-white dark:group-hover:text-black transition-colors duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
